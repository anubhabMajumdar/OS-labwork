...
main()
{
	...
	//------------------------------Output---------------------------------------//
		...
		int size_out, id_out, key_out=7778, i;
		...
		else
		{
		 ...
		 if (shared_mem->a[shared_mem->head]!=-1)
		 {
			 ...
			 if ((shared_output->tail)>=limit_input)
				...
				{
					if ((shared_output->a[(shared_output->tail)+1])==-1)
						break;
					else
						continue;
				}		
			
			shared_output->a[shared_output->tail]=answer;
			printf("Answer %d in position %d\n", shared_output->a[shared_output->tail], shared_output->tail);
			shared_output->tail++;
		}
