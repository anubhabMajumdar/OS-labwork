...
int main()
{
	...
	int shared_memory_id,process_id,*process_status,*shmget_status,shmget_id,array_point=0,q_size,p_size,p_select=0;
	...
	key_t key=0000;
		...
		process_status=(int *)shmat(process_id,NULL,0);		//allocated memory for modifying process status
		if (process_status<0)
			exit(1);
		...
		{	
			...
			for(array_point=0;array_point<q_size;array_point++)
			{
				//p_select=1;
					...
					while(process_status[p_select]!=0)
					{
						printf("\nProcess %d busy...\n",p_select);
						p_select++;
